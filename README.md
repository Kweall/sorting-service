...
* Архитектура: Интерфейс Repository расположен рядом с реализацией
* База данных: PostgreSQL с логикой сортировки на стороне БД
* API: POST эндпоинт для создания сортировок

## Технические детали:

* Сортировка выполняется на уровне PostgreSQL для оптимизации производительности
* Используется стандартная библиотека HTTP для минимальных зависимостей
* Интерфейс репозитория позволяет легко подменять реализацию хранилища